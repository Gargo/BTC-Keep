...
const UIEdgeInsets kSDSegmentedControlStainEdgeInsets = {0, 0, 0, 0};
const CGSize kSDSegmentedControlImageSize = {18, 18};

const CGFloat kSDSegmentedControlScrollOffset = 0;
...
- (void)commonInit
{
    ...
    {
        self.borderColor = UIColorPrimary;
...
// WARNING: the initial code was changed
- (void)selectSegmentIndex:(NSInteger)selectedSegmentIndex {
    
    self.selectedSegmentIndex = -1;
    if ([self._items count] > selectedSegmentIndex) {
        
        [self handleSelect:self._items[selectedSegmentIndex]];
    }
    else {
...
- (void)handleSelect:(SDSegmentView *)view
{
    ...
    if (index != NSNotFound && index != self.selectedSegmentIndex)
    {
        ...
        // WARNING: the initial code was changed
        [delegate segmentedControl:self didSelectItemAtIndex:self.selectedSegmentIndex];
        // end of change
    }
}
...
+ (void)initialize
{
    ...
    [appearance setTitleColor:[UIColor colorWithWhite:0.800 alpha:1] forState:UIControlStateNormal];
    [appearance setTitleColor:UIColorFromRGB(43, 154, 209) forState:UIControlStateSelected];
